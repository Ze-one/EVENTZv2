...
function getDesign(event: any) {
  const fallback = {
    backgroundColor: '#d8dcdf', topBarColor: '#d8dcdf', brandPanelColor: '#ffffff', textColor: '#020617', mutedTextColor: '#475569', logoBlockColor: '#0b1f4d', qrFrameColor: '#ffffff', primaryColor: event?.primaryColor || BRAND.navy, accentColor: event?.accentColor || BRAND.gold, slogan: BRAND.slogan, logoText: 'eventZ', customLogoDataUrl: '', logoFit: 'contain', showTopNotch: true, showBrandPanel: true, cornerRadius: 32
  };
  try {
...
function buildEmailHtml(participant: any, event: any, customMessage: string | undefined, qrImageUrl: string) {
  ...
  const logoHtml = design.customLogoDataUrl ? `<img src="${escapeHtml(design.customLogoDataUrl)}" width="96" height="64" alt="Logo" style="display:block;width:96px;height:64px;object-fit:${escapeHtml(design.logoFit || 'contain')};border:0;" />` : buildFallbackLogoHtml({ ...design, accentColor: accent });
...
export async function sendParticipantPassEmail(req: any, participant: any, event: any, logId: string, customMessage?: string) {
  ...
  const html = buildEmailHtml(participant, event, customMessage, qrImageUrl);
