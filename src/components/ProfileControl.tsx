...
type SafeUser = { id: string; name: string; email: string; role: UserRole; createdAt?: string };
...
export default function ProfileControl() {
  ...
  const updateProfile = async () => {
    ...
    setMessage('');
    try {
      const res = await fetch('/api/account-management', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: currentUser.id, name: profileName, email: profileEmail, password: profilePassword || undefined }) });
  ...
  return (
    <div className="fixed top-4 right-4 z-[120] font-sans text-left">
      <button onClick={() => setOpen((prev) => !prev)} className="relative bg-white/90 backdrop-blur-xl border border-white/70 shadow-2xl rounded-2xl pl-2 pr-3 py-2 flex items-center gap-2 hover:scale-[1.02]">
        ...
        <div className="w-9 h-9 rounded-xl bg-slate-950 text-yellow-400 flex items-center justify-center overflow-hidden font-black text-xs">{avatar ? <img src={avatar} alt="Profile" className="w-full h-full object-cover" /> : currentUser.name.slice(0, 2).toUpperCase()}</div>
      ...
      {open && (
        <div className="absolute right-0 mt-3 w-[min(92vw,460px)] max-h-[86vh] overflow-y-auto apple-card rounded-[2rem] p-4 animate-slide-up">
          ...
          <div className="mt-4 space-y-4">
            ...
            {isAdmin && (
              <section className="bg-white/70 rounded-3xl border border-slate-100 p-4 space-y-3">
                <h4 className="text-xs font-black text-slate-900 flex items-center gap-2"><Bell size={14} /> Admin Approval Requests <span className="bg-rose-100 text-rose-700 px-2 py-0.5 rounded-full text-[10px]">{pendingRequests.length}</span></h4>
                <p className="text-[11px] text-slate-500 leading-relaxed">Gate officers cannot directly add, delete, reset, or edit attendees. Their actions appear here for admin confirmation first.</p>
                <div className="space-y-2 max-h-56 overflow-y-auto no-scrollbar">
                  {pendingRequests.map((request) => (
                    <div key={request.id} className="bg-slate-50 border border-slate-100 rounded-2xl p-3 space-y-2">
                      <div><p className="text-xs font-black text-slate-800">{requestLabel(request)}</p><p className="text-[10px] text-slate-500">Requested by {request.requestedBy?.name || 'Gate Officer'} • {new Date(request.createdAt).toLocaleTimeString()}</p></div>
                      <div className="grid grid-cols-2 gap-2"><button onClick={() => reviewRequest(request.id, 'approved')} className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl py-2 text-[10px] font-black flex items-center justify-center gap-1"><CheckCircle2 size={12} /> Approve</button><button onClick={() => reviewRequest(request.id, 'rejected')} className="bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-xl py-2 text-[10px] font-black flex items-center justify-center gap-1"><XCircle size={12} /> Reject</button></div>
                    </div>
                  ))}
                  {pendingRequests.length === 0 && <p className="text-[11px] text-slate-400 text-center py-4">No pending gate officer requests.</p>}
                </div>
              </section>
            )}
            ...
            {isAdmin && (
              <section className="bg-white/70 rounded-3xl border border-slate-100 p-4 space-y-3">
                ...
                <p className="text-[11px] text-slate-500 leading-relaxed">Create separate gate accounts for multiple entry points. All gates validate against the same cloud pass state, preventing double validation.</p>
                <div className="grid gap-2"><input value={gateName} onChange={(e) => setGateName(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-semibold outline-none focus:ring-2 focus:ring-slate-950" placeholder="Gate officer name" /><input value={gateEmail} onChange={(e) => setGateEmail(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-semibold outline-none focus:ring-2 focus:ring-slate-950" placeholder="Gate login email" /><input type="password" value={gatePassword} onChange={(e) => setGatePassword(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs font-semibold outline-none focus:ring-2 focus:ring-slate-950" placeholder="Temporary password" /><button onClick={createGateOfficer} className="bg-yellow-400 hover:bg-yellow-300 text-slate-950 font-black py-3 rounded-2xl text-xs flex items-center justify-center gap-2"><UserPlus size={14} /> Create Gate Account</button></div>
                <div className="space-y-2 pt-2">{gateUsers.map((gate) => (<div key={gate.id} className="flex items-center justify-between gap-3 bg-slate-50 rounded-2xl border border-slate-100 p-3"><div className="min-w-0"><p className="text-xs font-black text-slate-800 truncate">{gate.name}</p><p className="text-[10px] text-slate-500 truncate">{gate.email}</p></div><button onClick={() => deleteGateOfficer(gate.id)} className="p-2 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-100"><Trash2 size={14} /></button></div>))}{gateUsers.length === 0 && <p className="text-[11px] text-slate-400 text-center py-3">No gate officer account found.</p>}</div>
              </section>
            )}
