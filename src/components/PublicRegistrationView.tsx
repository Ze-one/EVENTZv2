    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col">
        <header className="max-w-5xl w-full mx-auto px-5 py-6 flex items-center justify-between">
          ...
          <button onClick={onBack} className="text-xs font-bold text-slate-300 hover:text-white">Staff login</button>
        </header>
        <main className="flex-1 flex items-center justify-center px-5 pb-16">
          <div className="w-full max-w-xl bg-white text-slate-900 rounded-3xl p-8 shadow-2xl text-center space-y-6">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="max-w-6xl mx-auto px-5 py-6 flex items-center justify-between">
        <Logo size="md" variant="light" />
        <button onClick={onBack} className="flex items-center gap-2 text-xs font-bold text-slate-300 hover:text-white">
          <ArrowLeft size={14} /> Staff login
        </button>
      </header>

      <main className="max-w-6xl mx-auto px-5 pb-16 grid lg:grid-cols-[0.85fr_1.15fr] gap-8 items-start">
        <section className="space-y-6 pt-4">
          <div>
            <span className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-yellow-400 bg-yellow-400/10 border border-yellow-400/20 rounded-full px-3 py-1.5">
            ...
            <div className="mt-4 space-y-4">
              {[
                ...
                <div key={n} className="flex gap-3">
                  ...
                  <div><p className="text-xs font-black">{title}</p><p className="text-[11px] text-slate-400 mt-0.5 leading-relaxed">{body}</p></div>
                </div>
              ))}
            </div>
          </div>
